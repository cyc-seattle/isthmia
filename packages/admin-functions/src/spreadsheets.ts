...
export class Table<T extends Row> {
  ...
  public async addOrUpdate(predicate: (row: GoogleSpreadsheetRow<T>) => boolean, values: T) {
...
export class TypedSpreadsheet extends GoogleSpreadsheet {
  public async getOrCreateTable<T extends Row>(title: string, headerValues: RowKeys<T>) {
...
export class SpreadsheetClient {
  ...
  public async loadSpreadsheet(spreadsheetUrlOrId: string) {
    ...
    winston.debug("Loaded spreadsheet", {spreadsheetId, title: spreadsheet.title});
