...
export class ReportRunner {
  ...
  public async run(options: RunReportOptions) {
    ...
    const spreadsheet = await this.spreadsheets.loadSpreadsheet(options.spreadsheetUrl);
    const report = new reportClass(spreadsheet);
    report.run(options.arguments, options.sheet);
  }
  ...
  public async runAll(configSpreadsheetId: string) {
    ...
    const reports = await configSpreadsheet.getOrCreateTable<ReportRow>('Reports', ["enabled", "report", "arguments", "spreadsheetUrl", "sheet", "lastRun", "success"]);
