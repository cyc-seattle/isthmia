...
export const program = new Command('clubspot')
  .addOption(new Option("-u, --username <username>").env("CLUBSPOT_EMAIL").makeOptionMandatory())
  .addOption(new Option("-p, --password <password>").env("CLUBSPOT_PASSWORD").makeOptionMandatory())
  .addOption(new Option("--debug", "Enable debug-level logging").default(false).env("DEBUG"))
...
for(const schema of schemas) {
  ...
  subcommand.command("get <id>")
    .action(async (objectId, _options) => {
  ...
  subcommand.command("list")
    ...
    .option("--filter <attribute>=<value>", "Filters the results where attribute == value. Repeatable", parseFilterOption, [])
